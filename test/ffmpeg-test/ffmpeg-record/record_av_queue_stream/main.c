...
WRITE_AUDIO_FRAME:
 ...
 int choose_output(void)// 1:video 0:audio
{
    	int i;

   	 int64_t opts_audio = output_dev.audio_st->st->cur_dts == AV_NOPTS_VALUE ? INT64_MIN :
                       av_rescale_q( output_dev.audio_st->st->cur_dts, output_dev.video_st->st->time_base,
                                    AV_TIME_BASE_Q);

	   if (output_dev.audio_st->st->cur_dts == AV_NOPTS_VALUE)
	       printf( "audio cur_dts is invalid (this is harmless if it occurs once at the start per stream)\n");


	 int64_t opts_video = output_dev.video_st->st->cur_dts == AV_NOPTS_VALUE ? INT64_MIN :
                       av_rescale_q( output_dev.video_st->st->cur_dts, output_dev.video_st->st->time_base,
                                    AV_TIME_BASE_Q);

	   if (output_dev.video_st->st->cur_dts == AV_NOPTS_VALUE)
            	printf( "video cur_dts is invalid (this is harmless if it occurs once at the start per stream)\n");

        if (opts_video <= opts_audio) {
            return 1;
        }
...
int main(int argc, char **argv)
{
    ...
    while (output_dev.encode_video || output_dev.encode_audio) { //){//
		...
		printf("audio_st cur_pts=%s video_st cur_pts=%s\n",av_ts2str(output_dev.audio_st->st->cur_dts),av_ts2str(output_dev.video_st->st->cur_dts));

		if (output_dev.encode_video &&
		(!output_dev.encode_audio || choose_output() ) )//video
		{
