 ...
 AVPacket *get_audio_pkt(OutputStream *ost,IntputDev* input)
{
	...
	AVStream *in_stream;
	in_stream=input->a_ifmtCtx->streams[input->audioindex];
	...
	int64_t pkt_dts;
  	...
  	if(av_read_frame(input->a_ifmtCtx, input->in_packet)>=0){
		if(input->in_packet->stream_index==input->audioindex){
			...
			if (debug_ts) {
				av_log(NULL, AV_LOG_INFO,"audio pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s off:%s off_time:%s\n",
				av_ts2str(input->in_packet->pts), av_ts2timestr(input->in_packet->pts,&in_stream->time_base),
				av_ts2str(input->in_packet->dts), av_ts2timestr(input->in_packet->dts, &in_stream->time_base),
				av_ts2str(input->ts_offset),
				av_ts2timestr(input->ts_offset, &AV_TIME_BASE_Q));
    			}

			if (input->in_packet->dts != AV_NOPTS_VALUE)
				input->in_packet->dts += av_rescale_q(input->ts_offset, AV_TIME_BASE_Q,in_stream->time_base);
			if (input->in_packet->pts != AV_NOPTS_VALUE)
				input->in_packet->pts += av_rescale_q(input->ts_offset, AV_TIME_BASE_Q, in_stream->time_base);

			if (debug_ts) {
				av_log(NULL, AV_LOG_INFO,"111 audio pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s off:%s off_time:%s\n",
				av_ts2str(input->in_packet->pts), av_ts2timestr(input->in_packet->pts,&in_stream->time_base),
				av_ts2str(input->in_packet->dts), av_ts2timestr(input->in_packet->dts, &in_stream->time_base),
				av_ts2str(input->ts_offset),
				av_ts2timestr(input->ts_offset, &AV_TIME_BASE_Q));
			...
			if ( pkt_dts != AV_NOPTS_VALUE && input->last_ts != AV_NOPTS_VALUE) {
				...
				if (debug_ts) {
					av_log(NULL, AV_LOG_INFO,"111 audio delta:%s\n",
					av_ts2str(delta));
				}
			...
			{
				if (av_compare_ts(input->in_packet->pts, in_stream->time_base,
                      	STREAM_DURATION, (AVRational){ 1, 1 }) >= 0)
				{
					av_free_packet(input->in_packet);
		       		 return NULL;
	...
	if(pkt!=NULL)
	{
		...
		*got_pcm=got_picture;
		...
		if(got_picture){
			...
			frame->pts =av_rescale_q_rnd(pts, in_stream->time_base, ost->enc->time_base,AV_ROUND_UP);
				//ost->next_pts  += frame->nb_samples;
			...
			if(frame->nb_samples*4==input->out_buffer_size)//16bit stereo
			{
				...
				if (debug_ts) {
					av_log(NULL, AV_LOG_INFO,"222 audio pAudioFrame->pts:%s frame_pts:%s  frame_pts_time:%s \n",
					av_ts2str(input->pAudioFrame->pts),av_ts2str(frame->pts), av_ts2timestr(frame->pts,&ost->enc->time_base));
	    			}
			}
		}
	}
  ...
  int write_audio_frame1(AVFormatContext *oc, OutputStream *ost,AVFrame *in_frame)
{
	...
	if(ret < 0){
		printf("audio av_encode_frame Error.\n");
		//return NULL;
	}
